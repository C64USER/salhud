...
/**
 * Base class that represents a query for the 'town' table.
 *
 *
 *
 ...
 * @method     ChildTownQuery orderByMapping($order = Criteria::ASC) Order by the mapping column
 * @method     ChildTownQuery orderByRegionId($order = Criteria::ASC) Order by the region_id column
 *
 * @method     ChildTownQuery groupById() Group by the id column
 * @method     ChildTownQuery groupByName() Group by the name column
 * @method     ChildTownQuery groupByMapping() Group by the mapping column
 * @method     ChildTownQuery groupByRegionId() Group by the region_id column
 *
 ...
 *
 * @method     ChildTownQuery leftJoinRegion($relationAlias = null) Adds a LEFT JOIN clause to the query using the Region relation
 * @method     ChildTownQuery rightJoinRegion($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Region relation
 * @method     ChildTownQuery innerJoinRegion($relationAlias = null) Adds a INNER JOIN clause to the query using the Region relation
 *
 * @method     ChildTownQuery leftJoinTown($relationAlias = null) Adds a LEFT JOIN clause to the query using the Town relation
 * @method     ChildTownQuery rightJoinTown($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Town relation
 * @method     ChildTownQuery innerJoinTown($relationAlias = null) Adds a INNER JOIN clause to the query using the Town relation
 *
 * @method     \RegionQuery|\CaseQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildTown findOneByMapping(int $mapping) Return the first ChildTown filtered by the mapping column
 * @method     ChildTown findOneByRegionId(int $region_id) Return the first ChildTown filtered by the region_id column *
 ...
 * @method     ChildTown requireOneByMapping(int $mapping) Return the first ChildTown filtered by the mapping column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildTown requireOneByRegionId(int $region_id) Return the first ChildTown filtered by the region_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildTown[]|ObjectCollection findByMapping(int $mapping) Return ChildTown objects filtered by the mapping column
 * @method     ChildTown[]|ObjectCollection findByRegionId(int $region_id) Return ChildTown objects filtered by the region_id column
...
abstract class TownQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Base\TownQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'salhud', $modelName = '\\Town', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, mapping, region_id FROM town WHERE id = :p0';
        try {
    ...
    public function filterByMapping($mapping = null, $comparison = null)
    {
        if (is_array($mapping)) {
            ...
            if (isset($mapping['min'])) {
                $this->addUsingAlias(TownTableMap::COL_MAPPING, $mapping['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($mapping['max'])) {
                $this->addUsingAlias(TownTableMap::COL_MAPPING, $mapping['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TownTableMap::COL_MAPPING, $mapping, $comparison);
    }
    ...
    public function filterByRegionId($regionId = null, $comparison = null)
    {
        if (is_array($regionId)) {
            ...
            if (isset($regionId['min'])) {
                $this->addUsingAlias(TownTableMap::COL_REGION_ID, $regionId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($regionId['max'])) {
                $this->addUsingAlias(TownTableMap::COL_REGION_ID, $regionId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TownTableMap::COL_REGION_ID, $regionId, $comparison);
    }
    ...
    public function filterByRegion($region, $comparison = null)
    {
        if ($region instanceof \Region) {
            return $this
                ->addUsingAlias(TownTableMap::COL_REGION_ID, $region->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(TownTableMap::COL_REGION_ID, $region->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
    ...
    /**
     * Filter the query by a related \Case object
     *
     * @param \Case|ObjectCollection $case the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildTownQuery The current query, for fluid interface
     */
    public function filterByTown($case, $comparison = null)
    {
        if ($case instanceof \Case) {
            return $this
                ->addUsingAlias(TownTableMap::COL_ID, $case->getTownId(), $comparison);
        } elseif ($case instanceof ObjectCollection) {
            return $this
                ->useTownQuery()
                ->filterByPrimaryKeys($case->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByTown() only accepts arguments of type \Case or Collection');
        }
    }
    ...
    /**
     * Use the Town relation Case object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \CaseQuery A secondary query class using the current class as primary query
     */
    public function useTownQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTown($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Town', '\CaseQuery');
    }
    ...
    /**
     * Deletes all rows from the town table.
     *
